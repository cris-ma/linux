			   ...
			   struct payload __iomem *payld)
{
	...
	for (int i = 0; i < SCMI_TLM_DE_IMPL_MAX_DWORDS; i++)
		 ...
		 */
		uuid->de_impl_version[i] = ioread32be(&uuid_l->dwords[i]);
					...
					struct scmi_telemetry_group *grp)
{
	...
	for (int i = 0; i < grp->info->num_des; i++)
		scmi_telemetry_de_unlink(rinfo->des[grp->des[i]]);
}
				      ...
				      void *r, bool is_group)
{
	...
	if (IS_SHMTI_ID_VALID(sid)) {
		...
		if (!is_group) {
			...
			if (de_offs + de->info->data_sz >= ti->shmti[sid].info.len)
				return -EPROTO;
			...
			if (USE_BLK_TS(payld) && !tde->bts) {
				...
				if (bts_payld &&
				    SHMTI_OVERFLOWS(bts_payld, &ti->shmti[sid]))
					return -ENOSPC;
